use super::{
    components, peer_selection::FilterPeerSelector, FilterPeerSelectionConfig, BAD_MESSAGE_BAN_TIME,
};
...
pub struct FilterProtocol {
    ...
    pub(crate) peer_selector: FilterPeerSelector,
}

impl FilterProtocol {
    pub fn new(storage: Storage, peers: Arc<Peers>) -> Self {
        Self::new_with_peer_selection(storage, peers, FilterPeerSelectionConfig::default())
    }

    pub fn new_with_peer_selection(
        storage: Storage,
        peers: Arc<Peers>,
        peer_selection: FilterPeerSelectionConfig,
    ) -> Self {
        Self {
            storage,
            peers,
            last_ask_time: Arc::new(Default::default()),
            peer_selector: FilterPeerSelector::new(peer_selection),
        }
    }
    ...
    async fn should_ask(&self, immediately: bool) -> bool {
        if self.peer_selector.has_live_request() {
            return false;
        }
        ...
        !self.storage.is_filter_scripts_empty()
            && (immediately
                || last_ask.is_none()
                || last_ask.unwrap().elapsed() > self.peer_selector.request_timeout())
    }
    ...
    pub(crate) async fn try_send_get_block_filters(
        ...
        let mut candidates = self.peers.get_best_proved_peers(&tip_header);
        ...
        if let Some(peer) = self.peer_selector.choose(&nc, candidates) {
    ...
    pub(crate) async fn try_send_get_block_filter_hashes(&self, nc: BoxedCKBProtocolContext) {
            ...
            .await
        {
            let best_peers = self
                ...
                .collect::<Vec<_>>();
            if let Some(peer) = self.peer_selector.choose(&nc, best_peers) {
...
impl FilterProtocol {
    ...
    pub(crate) fn send_get_block_filters(
        ...
        let message = packed::BlockFilterMessage::new_builder()
            .set(content)
            .build();
        self.peer_selector.record_request(start_number, peer);
        if let Err(err) = nc.send_message_to(peer, message.as_bytes()) {
            self.peer_selector.record_send_failure(start_number, peer);
